...
public class SpecialDaoImpl extends ApplicationDaoImpl<SpecialEntity> implements SpecialDao {
  ...
  public PaginatedListTo<SpecialEntity> findSpecials(SpecialSearchCriteriaTo criteria) {
    ...
    WeeklyPeriodSearchCriteriaTo searchCriteria = criteria.getActivePeriod();
    if (searchCriteria != null) {
      query.where(Alias.$(special.getActivePeriod().getStartingDay()).eq(searchCriteria.getStartingDay()));
      query.where(Alias.$(special.getActivePeriod().getStartingHour()).eq(searchCriteria.getStartingHour()));
      query.where(Alias.$(special.getActivePeriod().getEndingDay()).eq(searchCriteria.getEndingDay()));
      query.where(Alias.$(special.getActivePeriod().getEndingHour()).eq(searchCriteria.getEndingHour()));
    }
